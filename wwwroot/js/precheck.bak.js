...
function extractCodigoGeneracionSync(raw) {
    ...
    try {
        ...
        if (p && UUID_RE.test(p)) return p.toUpperCase();
...
class PrechequeoManager {
    ...
    updateRankingDisplay(trips, levelInfo) {
        ...
        const elements = {
            ...
            percentage: document.querySelector('.nivel-porcentaje'),
            batteryFills: document.querySelectorAll('.battery-fill'),
            percentages: document.querySelectorAll('.nivel-porcentaje')
        };

        console.log('🔍 Elementos encontrados:', {
            trips: !!elements.trips,
            badge: !!elements.badge,
            batteryFills: elements.batteryFills ? elements.batteryFills.length : 0,
            percentages: elements.percentages ? elements.percentages.length : 0
        });
        ...
        if (elements.batteryFill && elements.percentage) {
            const levelConfig = this.getLevelConfig(levelInfo.percentage);
            console.log('✅ Actualizando batería SVG:', levelConfig);

            // Calcular el ancho basado en el porcentaje (máximo 54px de los 60px totales del rectángulo)
            ...
            const bottomY = bottomYAttr ? parseFloat(bottomYAttr) : 85;
            const maxHeight = maxHeightAttr ? parseFloat(maxHeightAttr) : 67;
            const fillHeight = Math.max(0, Math.min(maxHeight, (levelInfo.percentage / 100) * maxHeight));
            const newY = bottomY - fillHeight;

            // Actualizar el ancho del rectángulo de llenado con animación suave
            ...
            elements.percentage.textContent = `${levelInfo.percentage}%`;
        }
    ...
    animateVerticalFill(batteryFillEl, percentage) {
        try {
            const bottomY = parseFloat(batteryFillEl.getAttribute('data-bottom-y') || '85');
            const maxHeight = parseFloat(batteryFillEl.getAttribute('data-max-height') || '67');
    ...
    showDefaultRanking() {
        ...
        const nivelPorcentaje = document.querySelector('.nivel-porcentaje');
        ...
        if (nivelPorcentaje) {
            console.log('✅ Configurando porcentaje por defecto');
            nivelPorcentaje.textContent = '0%';
        }
    ...
    async tryGenericCamera() {
        ...
        try {
            ...
            Swal.fire({
                title: 'ERROR',
                text: 'No se pudo acceder a la cámara del dispositivo. Verifique los permisos de la aplicación.',
                icon: 'error',
                confirmButtonColor: '#3085d6',
                confirmButtonText: 'Aceptar',
                allowOutsideClick: false,
    ...
    takePhoto(event) {
        ...
        if (photo === 'data:,') {
            Swal.fire({
                title: 'ERROR',
                text: 'Por favor, capture una foto antes de continuar.',
                icon: 'error',
                confirmButtonColor: '#3085d6',
                confirmButtonText: 'Aceptar',
                allowOutsideClick: false,
    ...
    async uploadPhoto(photo, transactionCode) {
        try {
            ...
            if (response === 'success') {
                ...
                Swal.fire({
                    title: 'ERROR',
                    text: response || 'No se pudo subir la imagen al servidor',
                    icon: 'error',
                    allowOutsideClick: false,
                    allowEscapeKey: false
                });
