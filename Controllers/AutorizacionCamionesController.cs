...
{
    public class AutorizacionCamionesController : BaseController
    {
        private readonly IHttpClientFactory _httpClientFactory;
        private readonly ApiSettings _apiSettings;
        private readonly ILogger<AutorizacionCamionesController> _logger;
        ...
        public AutorizacionCamionesController(
            IHttpClientFactory httpClientFactory,
            IOptions<ApiSettings> apiSettings,
            ILogger<AutorizacionCamionesController> logger,
        ...
        private string ParseApiError(string errorContent, string defaultMessage = "Error en el servidor")
        {
            try
            {
                ...
                // Si no hay mensaje, buscar en otros campos comunes
            ...
            {
                // Si no se puede parsear como JSON, verificar si es un string simple
                if (!string.IsNullOrEmpty(errorContent))
                {
                    // Si contiene HTML, usar mensaje por defecto
        ...
        {
            var model = new AutorizacionCamionesViewModel();
            ...
            try
            {
                ...
                List<PostAutorizacion> dataStatus2 = new();
                if (responseStatus2.IsSuccessStatusCode)
                {
                    var content = await responseStatus2.Content.ReadAsStringAsync();
                    dataStatus2 = JsonConvert.DeserializeObject<List<PostAutorizacion>>(content) ?? new();
                }
                else
                {
                ...
                List<PostAutorizacion> dataStatus13 = new();
                if (responseStatus13.IsSuccessStatusCode)
                {
                    var content = await responseStatus13.Content.ReadAsStringAsync();
                    dataStatus13 = JsonConvert.DeserializeObject<List<PostAutorizacion>>(content) ?? new();
                }
                else
                {
                ...
                // COMBINAR todos los datos (mostrar siempre, con o sin fecha)
                var allData = dataStatus2.Concat(dataStatus13).ToList();

                // Procesamiento de fechas (convertir UTC -> GMT-6 cuando la fecha existe)
                ...
                // ASIGNAR A MODELO:
                // - Mostrar siempre las unidades operativas (status 2), independientemente de si tienen fecha
                var unidadesOperativas = allData
                    .Where(item => item.currentStatus == 2)
                    .ToList();

                model.UnidadesPlanas = unidadesOperativas
                    .Where(item => item.vehicle?.truckType == "R")
                    .OrderBy(item => item.dateTimePrecheckeo ?? DateTime.MaxValue)
                    .ToList();

                model.UnidadesVolteo = unidadesOperativas
                    .Where(item => item.vehicle?.truckType == "V")
                    .OrderBy(item => item.dateTimePrecheckeo ?? DateTime.MaxValue)
                    .ToList();

                model.CountPlanas = model.UnidadesPlanas.Count;
                model.CountVolteo = model.UnidadesVolteo.Count;

                // Mostrar siempre las inconsistencias (status 13), independientemente de si tienen fecha
                model.UnidadesInconsistencias = allData
                    .Where(item => item.currentStatus == 13)
                    .OrderBy(item => item.dateTimePrecheckeo ?? DateTime.MaxValue)
                    .ToList();

                // Logs para debugging (incluye conteos con y sin fecha)
                var totalWithValidDate = allData.Count(i => i.dateTimePrecheckeo.HasValue && i.dateTimePrecheckeo.Value != DateTime.MinValue);
                _logger.LogInformation($"Status 2 recibidos (raw): {dataStatus2.Count}");
                _logger.LogInformation($"Status 13 recibidos (raw): {dataStatus13.Count}");
                _logger.LogInformation($"Total combinado: {allData.Count}");
                _logger.LogInformation($"Total con fecha válida: {totalWithValidDate}");
                _logger.LogInformation($"Unidades Plana (status2): {model.CountPlanas}");
                _logger.LogInformation($"Unidades Volteo (status2): {model.CountVolteo}");
                _logger.LogInformation($"Inconsistencias (status13): {model.UnidadesInconsistencias.Count}");

                // Conteo por ingenios (incluir status 2 y 13) pero SOLO contar vehículos tipo R o V
                ...
                // Items para conteo: tomar todos los items con status 2, pero FILTRAR por truckType == "R" o "V"
                var itemsParaConteoIngenio = allData
                    .Where(item => item.currentStatus == 2
                                && (item.vehicle?.truckType == "R" || item.vehicle?.truckType == "V"))
                    .ToList();
            ...
            {
                _logger.LogError(ex, "Error al cargar datos de autorización de camiones");
            ...
            if (errores.Count == 0)
            {
                try
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        var data = JsonConvert.DeserializeObject<PostAutorizacion>(content);
        ...
        public async Task<IActionResult> GuardarReporteInconsistencia([FromBody] ReporteInconsistenciaRequest request)
        {
            ...
            try
            {
                ...
                if (userId == 0)
                {
                    ...
                    // Este caso no debería ocurrir porque el middleware ya validó la autenticación
                ...
                if (response.IsSuccessStatusCode)
                {
                    _logService.LogActivityAsync(codeGen ?? "", request, Usuario, 13);
        ...
        private void LogEvent(string message)
        {
            try
            {
                ...
                var logFilePath = Path.Combine(logDirectory, "MyAppLog.txt");
                var formattedLog = $"{DateTime.Now:yyyy-MM-dd HH:mm:ss} - {message}{Environment.NewLine}";
                
                System.IO.File.AppendAllText(logFilePath, formattedLog);
                _logger.LogInformation(message);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error al escribir en el log: {Message}", ex.Message);
            }
        }
    }
}